...
class StudentTrialExamPdfBuilder {
  final ValueNotifier<bool> notifier = ValueNotifier(false);
  final BuildContext context;

  StudentTrialExamPdfBuilder(this.context);
  ...
  Future<void> build() async {
    ...
    final image = context.read<StudentTrialExamDetailCubit>().imageBytes;
    ...
    //page.graphics.drawImage(PdfBitmap(image), const Rect.fromLTWH(0, 100, 30, 50));
    //Başlık
    ...
    //await _drawStaticsGraps(page, image);
    page.graphics.drawLine(PdfPen(PdfColor(165, 42, 42), width: 5), Offset(10, 100), Offset(10, 200));
  ...
  Future<void> _drawHeader(PdfPage page, Size pageSize) async {
    ...
    page.graphics.drawString(
        'DENEME SUNAVLARI ÖĞRENCİ SONUÇ KARNESİ', await PdfHelper.getPdfFont(size: 18, isBold: true),
  ...
  Future<void> _drawStudentInfoBox(PdfPage page, Student student) async {
    ...
    //Container
    final PdfGrid containerGrid = PdfGrid();
    containerGrid.columns.add(count: 2);
    PdfGridRow containerRow = containerGrid.rows.add();

    final PdfGrid studentInfoGrid = PdfGrid();
    studentInfoGrid.columns.add(count: 2);
    studentInfoGrid.columns[0].width = pageWidth / 2 * .4;
    studentInfoGrid.columns[1].width = pageWidth / 2 * .6;
    studentInfoGrid.style.cellPadding = PdfPaddings(left: 5, top: 5, bottom: 5, right: 5);
    PdfGridCellStyle cellTitleStyle = await PdfHelper.cellValueStyle(size: 8, isBold: true, isBorder: true);

    await _createTableRow(grid: studentInfoGrid, title: 'Sınıfı', value: student.className!, style: cellTitleStyle);
    await _createTableRow(
        grid: studentInfoGrid, title: 'Adı Soyadı', value: student.studentName!, style: cellTitleStyle);
    await _createTableRow(
        grid: studentInfoGrid, title: 'Numarası', value: student.studentNumber!, style: cellTitleStyle);
    await _createTableRow(grid: studentInfoGrid, title: 'Sınıf Sırası', value: '1', style: cellTitleStyle);
    await _createTableRow(grid: studentInfoGrid, title: 'Okul Sırası', value: '1', style: cellTitleStyle);

    containerRow.cells[0].value = studentInfoGrid;
    containerRow.cells[0].style = cellTitleStyle;

    containerRow.cells[1].style = cellTitleStyle;

    containerGrid.draw(
        page: page, bounds: Rect.fromLTWH(0, 40, page.getClientSize().width, page.getClientSize().height));
  }

  Future<void> _drawStaticsGraps(PdfPage page, Uint8List image) async {
    final PdfGrid grid = PdfGrid();
    grid.columns.add(count: 3);
    PdfGridRow row = grid.rows.add();
    row.cells[0].value = PdfBitmap(image);
    grid.draw(page: page, bounds: Rect.fromLTWH(0, 100, page.getClientSize().width, page.getClientSize().height));
  }

  Future<void> _createTableRow({
  ...
  Future<void> _drawStudentInfoBox2(PdfPage page, Student student) async {
    final pageWidth = page.getClientSize().width;
    //final pageHeight = page.getClientSize().height;

    debugPrint('Genişlik : ${pageWidth.toString()}');

    final PdfGrid grid = PdfGrid();
    grid.columns.add(count: 2);
    grid.columns[0].width = pageWidth * 1;
    grid.columns[1].width = pageWidth * 1;
    ...
    PdfGridCellStyle cellTitleStyleCenter = await PdfHelper.cellValueStyle(size: 8, isBold: true, isBorder: true);
